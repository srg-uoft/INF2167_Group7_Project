...
Before going into spatial and temporal trends, we first briefly look at how different vehicle types are represented in the dataset. From [@fig-numvehicles] below, we see that a vast, overwhelming majority of vehicles included in this dataset are automobiles.

![A Vast Majority of Vehicles Recorded Are Automobiles (Cars)](figures/number_of_vehicles.png){#fig-numvehicles}

Moving a level deeper and looking at what combinations of vehicles are involved in collisions ([@fig-combos]), we see that the automobile-heavy trend continues, with nearly 84% of collisions occuring between cars, with no other vehicle types involved. Another 11% occur between cars and public transit vehicles, with other combinations of the five vehicle types making up only small percentages of the overall data. 
