...
Generated models will use built-in `R` modelling functionality [@cite-R], visualization capabilities from the `broom` package [@cite-broom], and summary functions from the `modelsummary` package [@cite-modelsummary]. The performance of any generated models will be reported and discussed further in the sections below.

# Results & Findings

## Vehicle Type Analysis

Before going into spatial and temporal trends, we first look at how different vehicle types are represented in the dataset. From [@fig-numvehicles] below, we see that a vast, overwhelming majority of vehicles included in this dataset are automobiles.

![A Vast Majority of Vehicles Recorded (Nearly 86%) Are Automobiles (Cars)](figures/number_of_vehicles.png){#fig-numvehicles}

Moving a level deeper and looking at what combinations of vehicles are involved in collisions ([@fig-combos]), we see that the automobile-heavy trend continues, with nearly 84% of collisions occurring between cars, with no other vehicle types involved. Another 11% occur between cars and public transit vehicles, with other combinations of the five vehicle types making up only small percentages of the overall data. 

::: {#fig-combos layout-ncol=2}

![](figures/collision_combos_pt1.png)

![](figures/collision_combos_pt2.png)

Nearly All Collisions Involve Only Cars or Cars and Public Transit Vehicles
:::

## Temporal and Spatial Analysis

Overall, when looking at spatial and temporal data for vehicles involved in collisions, we see similar trends across vehicle type for collision hour and location, but notably diverging trends for collision day-of-week and collision month. Summary statistics are presented in [@fig-sums], and temporal trends are elaborated on in more depth below.

![Hour, Day-of-Week, Month, and Neighbourhood with the Most Collisions for All Vehicle Types](figures/fig4_summary_table.png){#fig-sums}

### Afternoon Peak for All Vehicle Types

Looking at [@fig-hourly], we see that trends in collision frequency are similar across all five vehicle types. Overnight collisions (21:00 - 04:00) are low, before a sharp increase between 05:00-09:00. We then see a steady increase throughout late morning and afternoon, a daily high between 15:00 and 18:00, and then a steady decline through the evening for all vehicle types. While collisions frequency for automobiles is substantially higher than for other vehicle types (looking at the differing y-axis scales for the various vehicle plots), the daily pattern remains the same.

![Collisions Per Hour are Fairly Consistent Across Vehicle Types](figures/fig2_collisions_by_hour.png){#fig-hourly}

### Weekday vs Weekend Trends

Zooming out to look at trends per day of the week in [@fig-weekly], there are two primary groups. Automobiles, bicycles, and pedestrians have low collision frequencies on weekend days (Saturday and Sunday) and higher numbers during the week, while motorcycles and public transit vehicles have more collisions clustered towards the end of the week and the weekend. Bicycles and pedestrians experience weekly peak collisions in the middle of the week (Tuesday - Thursday), while automobiles experience a steady increase throughout the week, a sharp peak on Friday, and then a sharp decrease during the weekend. Friday and Saturday are peak days for both motorcycles and public transit vehicles.

![Automobile, Pedestrian, and Bicycles Have More Weekday Collisions, while Motorcycles and Public Transit Vehicles Have More Weekend Collisions](figures/fig3_collisions_by_dow.png){#fig-weekly}

### Opposing Seasonal Trends

Considering collisions per month across the year in [@fig-monthly], similar to looking at day-of-week trends, two main patterns emerge. For bicycles and motorcycles, collisions sharply increase between April and May, stay high through September, and then drop sharply through the end of the year. However, for automobiles, pedestrians, and public transit vehicles, the opposite trend is true: collisions are at a yearly high towards the beginning of the year (for automobiles and public transit vehicles) or the end of the year (for pedestrians), see a sharp drop around April, and then steadily climb again to the end of the year.

![Bicycles and Motorcycles Experience Yearly Highes from April - August, While Automobiles/ Public Transit Vehicles, and Pedestrians Experience Yearly Lows](figures/fig1_collisions_by_month.png){#fig-monthly}

## Modeling Spatial, Temporal, and Vehicle Type Relationships

As mentioned in the previous section, linear regression models were run to better understand the relationships between the key variables to answering our research question. The first model, `overall_model`, was run using neighbourhood, hour, day-of-week, month, and vehicle type as predictors, attempting to predict the number of collisions that will happen in any combination of the predictor variables. The model as written in `R` syntax is:
...
With the data overwhelmingly containing automobile collisions, we wondered if the vehicle type variable might be throwing the model off, since the remaining vehicle types are so underrepresented and might not have sufficient data to generate meaningful correlations. Therefore, we ran a second model, `minus_vt_model`, to investigate if model performance would improve if vehicle type were removed as a predictor. Since in the EDA phase we determined that a vast majority of vehicle data points in the long data frame were automobiles, having such a disproportionate predictor might be skewing the model. The second model as written in `R` syntax is:
